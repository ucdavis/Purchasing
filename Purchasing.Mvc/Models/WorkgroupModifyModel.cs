...
namespace Purchasing.Mvc.Models
{
    ...
    public class WorkgroupModifyModel
    {
        ...
        public static WorkgroupModifyModel Create(User user, IQueryRepositoryFactory queryRepositoryFactory, Workgroup workgroup = null)
        {
            ...
            modifyModel.Organizations = workgroup != null ? workgroup.Organizations.Select(x => new SelectListItem(x.Name + " (" + x.Id + ")", x.Id, true) {Selected = true}).ToList() : new List<SelectListItem>();

            var userOrgs = user.Organizations.Where(x => !modifyModel.Organizations.Select(y => y.Value).Contains(x.Id));
            modifyModel.Organizations.AddRange(userOrgs.Select(x => new SelectListItem(x.Name + " (" + x.Id + ")", x.Id, true)));
            ...
            modifyModel.Organizations.AddRange(decendantOrgs.Select(x => new SelectListItem(x.Name + " (" + x.OrgId + ")", x.OrgId, true)));
