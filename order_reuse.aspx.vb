...
Partial Class order_reuse
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            Dim cmd As DbCommand = _db.GetStoredProcCommand("order_reuse")
            ...
            sdsPreOrderInfo.SelectParameters("vendor_id").DefaultValue = dr("vendor_id")
            hdVendorID.Value = dr("vendor_id")

            ddlAuthorizer.DataBind()
            If ddlAuthorizer.Items.Contains(ddlAuthorizer.Items.FindByValue(dr("routed_to"))) Then
                ddlAuthorizer.SelectedValue = dr("routed_to")
            End If

            ddlShipping.SelectedValue = "Least Expensive"

            If dr("radioactive") Then
                ckbxRadioactive.Checked = True
                If Not IsDBNull(dr("rua_num")) Then
                    txtRUA.Text = dr("rua_num")
                End If
            End If

            If dr("controlled_substance") Then
                ckbxControlledSubstance.Checked = True
                pnlCS.Visible = True
                If Not IsDBNull(dr("cs_class")) Then
                    txtCSClass.Text = dr("cs_class")
                End If
                If Not IsDBNull(dr("cs_use")) Then
                    txtCSUse.Text = dr("cs_use")
                End If
                If Not IsDBNull(dr("cs_store")) Then
                    txtCSStore.Text = dr("cs_store")
                End If
                If Not IsDBNull(dr("cs_custodian")) Then
                    txtCSCustodian.Text = dr("cs_custodian")
                End If
                If Not IsDBNull(dr("cs_user")) Then
                    txtCSUser.Text = dr("cs_user")
                End If
            End If

            If Not IsDBNull(dr("vendor_contact")) Then
                txtVendor.Text = dr("vendor_contact")
            End If
            If Not IsDBNull(dr("request_comments")) Then
                txtComment.Text = dr("request_comments")
            End If
            ...
            While dr.Read()
                ...
                tqty.Text = dr("quantity")
                tunit.Text = dr("unit")
                If Not IsDBNull(dr("item")) Then
                    titem.Text = dr("item")
                End If
                tdesc.Text = dr("description")
                If Not IsDBNull(dr("exceed")) Then
                    texceed.Text = String.Format("{0:f2}", dr("exceed"))
                End If
                If Not IsDBNull(dr("cost")) Then
                    tcost.Text = String.Format("{0:f2}", dr("cost"))
                End If
