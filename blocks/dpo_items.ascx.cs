...
public partial class business_purchasing_blocks_dpo_items : System.Web.UI.UserControl
{
    ...
    protected void HandleIE()
    {
        ...
        {
            foreach (System.Web.UI.Control c in dpoItemsPanel.Controls)
            {
    ...
    protected void InitialDpoRows(int count)
    {
        for (int r = 0; r <= 14; r++)
        {
            HtmlTableRow tr = dpoItemsPanel.FindControl("Tr" + r.ToString()) as HtmlTableRow;
            if (r < (count))
    ...
    public void PopulateItems(int orderID)
    {
        ...
        Purchasing.dpo_itemsDataTable dt = dpoItemsAdapter.GetByOrderId(orderID);

        int r = 0;
        double rowsSubTotal = 0;

        foreach (Purchasing.dpo_itemsRow dr in dt.Rows)
        {
            HtmlTableRow tr = dpoItemsPanel.FindControl("Tr" + r.ToString()) as HtmlTableRow;
            HiddenField hf = dpoItemsPanel.FindControl("rowHiddenField" + r.ToString()) as HiddenField;
            RadNumericTextBox qty = dpoItemsPanel.FindControl("qtyRadNumericTextBox" + r.ToString()) as RadNumericTextBox;
            TextBox unit = dpoItemsPanel.FindControl("unitTextBox" + r.ToString()) as TextBox;
            TextBox catalogNum = dpoItemsPanel.FindControl("catalogTextBox" + r.ToString()) as TextBox;
            TextBox desc = dpoItemsPanel.FindControl("descTextBox" + r.ToString()) as TextBox;
            RadNumericTextBox unitPrice = dpoItemsPanel.FindControl("costRadNumericTextBox" + r.ToString()) as RadNumericTextBox;
            TextBox notes = dpoItemsPanel.FindControl("notes" + r.ToString() + "TextBox") as TextBox;

            tr.Style["display"] = String.Empty;
            qty.Text = dr.qty.ToString();
            unit.Text = dr.unit;
            catalogNum.Text = dr.catalog_num;
    ...
    public string GetHTMLTable()
    {
        ...
        emailTable += "<td style='" + style + " width:100px'><strong>Catalog #</strong></td>";
        emailTable += "<td style='" + style + "'><strong>Description</strong></td>";
        emailTable += "<td style='" + style + " width:55px'><strong>Unit Price</strong></td>";
        emailTable += "<td style='" + style + "'><strong>Notes</strong></td>";
        emailTable += "</tr>";
        ...
        for (int r = 0; r <= 14; r++)
        {
            ...
            if (!IsRowEmpty(qty, unit, catalogNum, desc, unitPrice))
            {
                emailTable += GetHTMLRow(qty.Text, unit.Text, catalogNum.Text, desc.Text, unitPrice.Text, notes.Text.Trim());
                subTotal += Convert.ToInt32(qty.Text) * Convert.ToDouble(unitPrice.Text);
            }
        }
        ...
        <td colspan='2' style='color:Green; border-bottom: 1px solid #000000; border-right: 1px solid #000000;'>
    ...
    /// <returns></returns>
    protected string GetHTMLRow(string qty, string unit, string catalogNum, string desc, string unitPrice, string notes)
    {
    ...
    public void InsertItems(int orderID)
    {
        ...
        for (int r = 0; r <= 14; r++)
        {
            ...
            if (!IsRowEmpty(qty, unit, catalogNum, desc, unitPrice))
            {
                InsertRow(orderID, qty.Text, unit.Text.Trim(), catalogNum.Text.Trim(), desc.Text.Trim(), unitPrice.Text, notes.Text.Trim());
                subTotal += Convert.ToInt32(qty.Text) * Convert.ToDouble(unitPrice.Text);
            }
        }
    ...
    /// <param name="desc"></param>
    /// <param name="unitPrice"></param>
    protected void InsertRow(int orderID, string qty, string unit, string catalogNum, string desc, string unitPrice, string notes)
    {
        PurchasingTableAdapters.dpo_itemsTableAdapter dpoItemsAdapter = new PurchasingTableAdapters.dpo_itemsTableAdapter();
        dpoItemsAdapter.Insert(
            orderID,
            Convert.ToInt32(qty),
            unit,
            catalogNum,
