...
-- =============================================
CREATE FUNCTION [dbo].[udf_GetOpenOrdersForLogin_Old]
  (
  ...
  BEGIN
    ...
    from (
         ...
         select distinct o.id                 orderid,
                         case
                           when ap.userid is null then wp.userid
         ...
         from orders o
                ...
                left outer join workgrouppermissions wp
                  on o.workgroupid = wp.workgroupid and ap.orderstatuscodeid = wp.roleid
         ...
         select o.id orderid, userid, isadmin, wp.roleid, 1 Edit, 1 [Read]
         from orders o
         ...
         select ap.OrderId,
                ap.SecondaryUserId accessuserid,
                cast(0 as bit)     isadmin,
                ap.OrderStatusCodeId,
                1                  Edit,
                1                  [Read]
         from approvals ap
         ...
         where ap.SecondaryUserId = @LoginId
         ...
         select ap.OrderId, ap.UserId accessuserid, cast(0 as bit) isadmin, ap.OrderStatusCodeId, 1 Edit, 1 [Read]
         from approvals ap
                inner join orders o on ap.OrderId = o.id
                inner join OrderStatusCodes aposc on ap.OrderStatusCodeId = aposc.id
                inner join OrderStatusCodes oosc on o.orderstatuscodeid = oosc.id
         ...
         select orderid, userid accessuserid, 0 [admin], ordertracking.OrderStatusCodeId, 0 Edit, 1 [Read]
         from ordertracking
         ...
         from workgrouppermissions wp
                ...
                inner join OrderStatusCodes osc on o.orderstatuscodeid = osc.id
         where wp.userid = @LoginId
           and wp.roleid = 'RV'
             --and o.orderstatuscodeid not in ('CN', 'CP', 'OC', 'OD')
           and osc.id not in ('CN', 'CP', 'OC', 'OD')) vopenaccess
