    ...
    public class LineItemRepositoryTests : AbstractRepositoryTests<LineItem, int, LineItemMap>
    {
        ...
        public void TestLineItemNewCommodityDoesNotSave()
        {
            ...
            catch (Exception ex)
            {
                Assert.IsTrue(thisFar);
                Assert.IsNotNull(ex);
                Assert.AreEqual("object references an unsaved transient instance - save the transient instance before flushing or set cascade action for the property to something that would make it autosave. Type: Purchasing.Core.Domain.Commodity, Entity: Purchasing.Core.Domain.Commodity", ex.Message);
                throw;
            }
        }
