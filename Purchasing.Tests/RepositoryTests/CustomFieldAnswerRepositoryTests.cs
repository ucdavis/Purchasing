    ...
    public class CustomFieldAnswerRepositoryTests : AbstractRepositoryTests<CustomFieldAnswer, int, CustomFieldAnswerMap>
    {
        ...
        public void TestCustomFieldAnswerNewCustomFieldDoesNotSave()
        {
            ...
            catch (Exception ex)
            {
                Assert.IsTrue(thisFar);
                Assert.IsNotNull(ex);
                Assert.AreEqual("object references an unsaved transient instance - save the transient instance before flushing. Type: Purchasing.Core.Domain.CustomField, Entity: Purchasing.Core.Domain.CustomField", ex.Message);
                throw;
            }
        }
