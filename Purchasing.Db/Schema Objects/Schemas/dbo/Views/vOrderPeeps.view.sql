...
select ROW_NUMBER() over (order by orderid) id, orderid, workgroupid, orderstatuscodeid, userid, fullname, administrative, sharedorcluster
from (
select orders.id orderid, orders.workgroupid, approvals.OrderStatusCodeId, wp.userid, users.firstname + ' ' + users.lastname fullname, 0 administrative, 0 sharedorcluster
from orders 
...
select orders.id orderid, orders.WorkgroupId, approvals.OrderStatusCodeId, admins.userid, admins.fullname, 1 administrative, admins.SharedOrCluster
from orders
	inner join approvals on approvals.OrderId = orders.id
	inner join (
