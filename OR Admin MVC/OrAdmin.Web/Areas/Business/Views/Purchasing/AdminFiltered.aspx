...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
    <h1>
        ...
        Requests (<%= Model.Filter.ToString().Replace("_", " ")%>) </h1>
    <% if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.All)
       { %>
    <p class="desc">A complete listing of <strong>all</strong>
        <%= Model.Unit.UnitAbbreviation %>
        requests.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Awaiting_PI)
       { %>
    <p class="desc">Requests that are being reviewed by the PI, but are not yet approved.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Awaiting_Manager)
       { %>
    <p class="desc">Requests that have been approved by the PI and are awaiting approval by an account manager.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Manager_Approved)
       { %>
    <p class="desc">Requests that have been approved by both the PI and account manager and are ready to be placed with the vendor.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Placed)
       { %>
    <p class="desc">Requests that have been <strong>placed</strong> with the vendor.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Not_Received)
       { %>
    <p class="desc">Requests that were <strong>not received</strong> by the requester.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Conflict)
       { %>
    <p class="desc">Requests that had a shipping or vendor <strong>conflict</strong>.</p>
    <% }
       else if (Model.Filter == OrAdmin.Repositories.Purchasing.RequestRepository.AdminRequestFilter.Received)
       { %>
    <p class="desc">Requests that were received by the requester.</p>
    <% } %>
