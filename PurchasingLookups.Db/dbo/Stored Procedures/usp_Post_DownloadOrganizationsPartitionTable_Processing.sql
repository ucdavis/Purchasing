...
BEGIN
	...
	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[WorkgroupsXOrganizations]'') AND type in (N''U''))
	...
	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[ConditionalApproval]'') AND type in (N''U''))
