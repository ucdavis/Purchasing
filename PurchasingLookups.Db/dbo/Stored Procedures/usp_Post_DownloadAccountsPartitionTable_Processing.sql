...
BEGIN
	...
	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[Splits]'') AND type in (N''U''))
	...
	IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[dbo].[WorkgroupAccounts]'') AND type in (N''U''))
	...
	IF NOT EXISTS (SELECT * FROM [PrePurchasingLookups].[sys].[fulltext_catalogs] WHERE [name] LIKE ''' + @TableName + '%'')
